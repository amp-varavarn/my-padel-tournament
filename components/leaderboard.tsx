...
export function Leaderboard({
  ...
  const roundGroups = matchHistory.reduce<Record<number, MatchResult[]>>((acc, result) => {
  ...
  // Accordion state -- track which rounds are expanded
        ...
        {isFinal && sorted.length > 0 && (
          <div className="flex flex-col items-center rounded-2xl bg-primary px-6 py-8 text-center">
            ...
            <p className="mt-1 text-sm text-primary-foreground/70">
              {sorted[0].wins} {sorted[0].wins === 1 ? "win" : "wins"} &middot;{" "}
                                ...
                                <div className="flex items-baseline justify-between gap-2">
                                  <span
                                    className={`text-sm truncate ${
                                      t1Won ? "font-semibold text-foreground" : "text-muted-foreground"
                                    }`}
                                  >
                                  ...
                                  <span
                                    className={`shrink-0 font-mono text-sm tabular-nums ${
                                      t1Won ? "font-bold text-foreground" : "text-muted-foreground"
                                    }`}
                                  >
                                    {result.score1}
                                  </span>
